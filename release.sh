...
STANDALONE_DEST="../firebase-clients/libs/emberfire"
STANDALONE_STUB="emberfire"


#############################
#  VALIDATE emberfire REPO  #
#############################
# Ensure the checked out emberfire branch is master
CHECKED_OUT_BRANCH="$(git branch | grep "*" | awk -F ' ' '{print $2}')"
if [[ $CHECKED_OUT_BRANCH != "master" ]]; then
  echo "Error: Your emberfire repo is not on the master branch."
  exit 1
fi

# Make sure the emberfire branch does not have existing changes
if ! git --git-dir=".git" diff --quiet; then
  echo "Error: Your emberfire repo has existing changes on the master branch. Make sure you commit and push the new version before running this release script."
  exit 1
fi
...
README_VERSION="$(grep '<script src=\"https://cdn.firebase.com/libs/emberfire/' README.md | awk -F '/' '{print $6}')"
...
#  PUBLISH TO Bower  #
######################
# Pull any changes to the emberfire repo
git pull origin master
if [[ $? -ne 0 ]]; then
  echo "Error: Failed to do 'git pull' from emberfire repo."
  exit 1
fi
...
git tag v$VERSION
if [[ $? -ne 0 ]]; then
  echo "Error: Failed to do 'git tag' from emberfire repo."
  exit 1
fi
...
git push --tags
if [[ $? -ne 0 ]]; then
  echo "Error: Failed to do 'git push --tags' from emberfire repo."
  exit 1
fi
...
echo "*** v${VERSION} of emberfire published to npm ***"
echo
...
git commit -am "[firebase-release] Updated EmberFire to $VERSION"
if [[ $? -ne 0 ]]; then
...
echo "  2) Update the release notes for EmberFire version ${VERSION} on GitHub"
echo "  3) Update all EmberFire client version numbers specified in firebase-website to ${VERSION}"
echo "  4) Tweet @FirebaseRelease: 'v${VERSION} of EmberFire is available https://cdn.firebase.com/libs/emberfire/$VERSION/emberfire.min.js Changelog: https://cdn.firebase.com/libs/emberfire/changelog.txt'"
echo
echo "Done! Woot!"
echo
