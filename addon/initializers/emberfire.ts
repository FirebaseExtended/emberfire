...
const initialize = (application: any) => {
  ...
  if (typeof environment.firebase.length === 'undefined') {
    ...
    environment.firebase.forEach((config: any) =>
      loadEnvironment(application, config)
    );
  }
  application.register(
    'service:realtime-listener',
    RealtimeListenerService.extend({}),
    { instantiate: true }
  );
...
const loadEnvironment = (application: any, environment: any) => {
  ...
  const serviceName =
    (options.name === '[DEFAULT]' && `firebase-app`) ||
    `firebase-${options.name}`;
  application.register(
    `service:${serviceName}`,
    FirebaseAppService.extend({ name: options.name }),
    { instantiate: true }
  );
};
