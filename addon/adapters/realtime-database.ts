 ...
 *
 */
export default class RealtimeDatabaseAdapter extends Adapter {
  ...
  database: RSVP.Promise<database.Database> | undefined = undefined;
  ...
  findHasMany<K extends keyof ModelRegistry>(
    ...
    if (adapter !== this) {
      ...
      return rootCollection(this, relationship.type).then((ref) =>
        queryDocs(
  ...
  findBelongsTo<K extends keyof ModelRegistry>(
    ...
    if (adapter !== this) {
      ...
      return docReference(this, relationship.type, snapshot.id).then((ref) =>
        ref.once('value')
      );
    }
  }
  ...
  query<K extends keyof ModelRegistry>(
    ...
    return rootCollection(this, type).then((ref) =>
      queryDocs(ref, queryOptionsToQueryFn(options))
    );
  }
  ...
  queryRecord<K extends keyof ModelRegistry>(
    ...
    const query = rootCollection(this, type).then((ref) =>
      queryDocs(ref.limitToFirst(1), queryOptionsToQueryFn(options))
    );
  ...
  createRecord<K extends keyof ModelRegistry>(
    ...
    const data = this.serialize(snapshot, { includeId: false });
    if (id) {
      return docReference(this, type, id).then((ref) =>
        ref.set(data).then(() => ({ ref, data }))
      );
    } else {
...
const queryOptionsToQueryFn =
  (options?: QueryOptions) => (collectionRef: database.Reference) => {
    ...
    if (options) {
      ...
      if (options.equalTo !== undefined) {
        ref =
          options.equalTo && typeof options.equalTo === 'object'
            ? ref.equalTo(options.equalTo[0], options.equalTo[1])
            : ref.equalTo(options.equalTo);
      }
      if (options.startAt !== undefined) {
        ref =
          options.startAt && typeof options.startAt === 'object'
            ? ref.startAt(options.startAt[0], options.startAt[1])
            : ref.startAt(options.startAt);
      }
      if (options.endAt !== undefined) {
        ref =
          options.endAt && typeof options.endAt === 'object'
            ? ref.endAt(options.endAt[0], options.endAt[1])
            : ref.endAt(options.endAt);
      }
...
export const rootCollection = (adapter: RealtimeDatabaseAdapter, type: any) =>
  databaseInstance(adapter).then((database) =>
    database.ref(
      [get(adapter, 'namespace'), collectionNameForType(type)].join('/')
    )
  );
const getDocs = (query: ReferenceOrQuery) =>
  query.once('value').then((value) => ((value as any).query = query) && value);
const docReference = (
  adapter: RealtimeDatabaseAdapter,
  type: any,
  id: string
