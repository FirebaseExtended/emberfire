...
//@ts-ignore
const getService = (object: Object) =>
  //@ts-ignore
  getOwner(object).lookup(
    'service:realtime-listener'
  ) as RealtimeListenerService;
...
function isFirestoreDocumentRefernce(
  arg: any
): arg is firestore.DocumentReference {
...
export default class RealtimeListenerService extends Service.extend({
  ...
  subscribe(route: Object, model: any, parentModel?: any, relationship?: any) {
    ...
    const ref = (model.get('meta._ref') ||
      model.get('_recordData._data._ref') ||
      model.get('_internalModel._recordData._data._ref')) as
    ...
    const observeRelationships = (internalModel: any) => {
      ...
      if (!movedMethod) {
        ...
        internalModel._updatePromiseProxyFor = (
          kind: string,
          key: string,
          args: Object
        ) => {
          ...
          proxy.then((result: any) => {
            if (internalModel[requestedRelationshipsKey].indexOf(key) < 0) {
              ...
              internalModel[proxyPromiseListenersKey].forEach((f: any) =>
                f(kind, key, args, result)
              );
            }
          });
          return proxy;
    ...
    let content =
      model.content ||
      (parentModel && get(parentModel, `${relationship.key}.content`));
    if (model._internalModel) {
      observeRelationships(model._internalModel);
    } else if (content) {
      // TODO find backing content for hasMany
    ...
    if (ref) {
      if (isFirestoreDocumentRefernce(ref)) {
        ...
        const unsubscribe = ref.onSnapshot((doc) => {
          run(() => {
            const normalizedData = serializer.normalizeSingleResponse(
              store,
              modelClass,
              doc
            );
      ...
      if (serializer.constructor.name == 'FirestoreSerializer') {
        ...
        const queryOrRoot =
          (query && resolve(query)) ||
          firestoreRootCollection(adapter, modelName);
        ...
        // RTDB findAll
        const ref = (model.get('meta.query') ||
          model.get('recordData._data._ref')) as database.Reference | undefined;
        const refOrRoot = ref
          ? resolve(ref)
          : realtimeDatabaseRootCollection(adapter, modelName);
        refOrRoot.then((ref) => {
          ...
          const onChildRemoved = ref.on('child_removed', (snapshot) => {
            run(() => {
              if (snapshot) {
                const record = content.find(
                  (record: any) => record.id === snapshot.key
                );
                if (record) {
